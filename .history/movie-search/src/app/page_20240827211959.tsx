...
const Home = () => {
  ...
  const handleRate = async (movieId: number, rating: number) => {
    ...
    try {
      ...
      console.error('Failed to rate movieThe issue with having multiple session IDs could indeed cause problems with your application, as it can lead to inconsistencies when fetching rated movies. The updated approach I've provided ensures that only one session ID is generated and used consistently across the application.
        const handleRate = async (movieId: number, rating: number) => {
          if (!sessionId) return;
          try {
            const response = await axios.post(
