...
const Home = () => {
  ...
  const fetchRatedMovies = async () => {
    ...
    try {
      ...
      console.log('Rated Movies:', response.data.results); // Check if movies are fetched
  ...
  const renderMovies = (movies: Movie[]) => {
    console.log('Rendering movies:', movies); // Check if ratedMovies is passed correctly
    return (
      <Row gutter={[16, 16]}>
        {movies.map((movie) => (
