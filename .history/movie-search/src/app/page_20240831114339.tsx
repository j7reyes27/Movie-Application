...
const Home = () => {
  ...
  const handleRate = async (movieId: number, rating: number) => {
    ...
    try {
      ...
      // Update rated movies state immediately
      setRefreshRatedMovies((prev) => !prev);
  
      // Optionally, manually add the movie to the ratedMovies list
      setRatedMovies((prevMovies) => {
        const alreadyRated = prevMovies.some(movie => movie.id === movieId);
        if (alreadyRated) {
          return prevMovies.map(movie =>
            movie.id === movieId ? { ...movie, userRating: rating } : movie
          );
        } else {
          const ratedMovie = movies.find(movie => movie.id === movieId);
          return ratedMovie ? [...prevMovies, { ...ratedMovie, userRating: rating }] : prevMovies;
        }
      });
    } catch (err) {
